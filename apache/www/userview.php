...
function get_users($conn, $check_out_of_office=false) { 
	...
	if ($result->num_rows > 0) {
	    while($row = $result->fetch_assoc()) {
			...
			array_push($data, $row['overjumps_sum']);
...
<nav class="navbar navbar-inverse navbar-fixed-top">
 ...
 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
    	...
    	<li  class="active"><a href="userview.php">Points<span class="sr-only">(current)</span></a></li>
...
<?php
	$users = get_users_in_order($conn);
		echo '<tr>';
			...
			echo '<th>Put Out</th>'; 
			echo '<th>Put In</th>'; 
			echo '<th>Paper Trash</th>'; 
			echo '<th>Jumped</th>';
		echo '<tr>';

	foreach ($users as $user) {
		$muell_user_id = get_curent_user($conn, 1);
		$pfand_user_id = get_curent_user($conn, 2);
		$aus_user_id = get_curent_user($conn, 3);
		$ein_user_id = get_curent_user($conn, 4);
		$kaffee_user_id = get_curent_user($conn, 5);
		...
		if ($user['out_of_office'] == 1) {
			$format1 = '';
			$format2 = '';
			$format3 = ' style="color:#AAAAAA;" ';
		}

		if ( $muell_user_id == $user['id'] or $pfand_user_id == $user['id'] or $aus_user_id == $user['id'] or $ein_user_id == $user['id'] or $kaffee_user_id == $user['id']) {
			echo '<tr bgcolor="#ffeecc">'.$format1;
		} else {
			echo '<tr'.$format3.'>';
		}
			...
			echo '<td><b>'.$format1.$user['first_name'].' '.$user['last_name'].$format2.'</b></td>'; 
			echo '<td align="center">'.$format1.$user['count_muell'].$format2.'</td>';
			echo '<td align="center">'.$format1.$user['count_pfand'].$format2.'</td>';
			echo '<td align="center">'.$format1.$user['count_aus'].$format2.'</td>';
			echo '<td align="center">'.$format1.$user['count_ein'].$format2.'</td>';
			echo '<td align="center">'.$format1.$user['count_kaffee'].$format2.'</td>';
			echo '<td align="center">'.$format1.$user['overjumps_sum'].$format2.'</td>';
		echo '</tr>';
	}
